...
public class BankTransactionGenerator {
    ...
    public BankTransactionGenerator(Config config, StringRedisTemplate redis, StatefulRediSearchConnection<String, String> connection)
        ...
        random.setSeed(config.getAccount().getSeed().getBytes("UTF-8")); // Prime the RNG so it always generates the same pseudorandom set
    ...
    private void createBalance() {
        ...
        redis.opsForZSet().add(BALANCE_TS, "0", System.currentTimeMillis());
        LOGGER.info("Created {} balances set", BALANCE_TS);
    }
    ...
    private BankTransaction createBankTransaction() {
        ...
        transaction.setToAccountName(config.getAccount().getName());
        transaction.setToAccount(Utilities.generateFakeIbanFrom(config.getAccount().getName()));
    ...
    private String createTransactionAmount(String accountName) {
        ...
        redis.opsForZSet().add(BALANCE_TS, Double.toString(balance), System.currentTimeMillis());
